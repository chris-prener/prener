...
ggsave("hwy", p, device = "png", 
       width = prener::cp_points(500, units = "mm"), 
       height = prener::cp_points(375, units = "mm"), 
       units = "mm", dpi = 300)
```
...
prener::cp_plotSave("hwy", p, preset = "sm")
prener::cp_plotSave("hwy", p, preset = "med")
prener::cp_plotSave("hwy", p, preset = "lg", dpi = 500)
```

If no `device` is specified, plots will be exported as `.png` files. Other output options may be specified using `device`:

```{r, eval=FALSE}
prener::cp_plotSave("hwy", p, device = "pdf", preset = "lg", dpi = 500)
```
